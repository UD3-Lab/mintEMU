...
-   Evaluating the number of topics quantitatively can be done through the measure of **topic coherence**
    -   Calculate topic coherence for K values ranging from 2 to K
    -   Plot K values against coherence scores and select K with the highest coherence value

A K with a smaller value will yield more general topics whereas a K with a high value will result in more specific topics. Hence, finding a suitable value entails a trade-off between interpretability and specificity. This is very much dependent on the data at hand and the research question.

#### Determining the value of K empirically
...
Fit model with K determined in previous step...
...
Run a cluster dendrogram (Hellinger distance) to see which topics should be grouped together

```{r}
# Perform hierarchical cluster analysis.

rownames(beta_k) <- topic_names_k
dend <- as.dist(beta_k) |> 
  hclust() |> 
  as.dendrogram()

# Plot with Color in function of the cluster
leafcolor <- colormap::colormap(colormap = colormap::colormaps$viridis, nshades = 5, format = "hex", alpha = 1, reverse = FALSE)
dend |> 
...
dendr    <- ggdendro::dendro_data(dend, type="rectangle") # convert for ggplot
clust    <- cutree(as.dist(beta_k) |> hclust(),k = 5)                    # find 2 clusters
...
ggplot() + 
  ...
  geom_text(data=ggdendro::label(dendr), aes(x, y - 0.002, label=label, hjust=0, color=cluster), 
           size=3) +
  coord_flip() + 
...
#### Determining the value of K qualitatively
