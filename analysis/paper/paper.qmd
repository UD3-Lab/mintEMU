...
The number of topics was empirically determined with the `ldatuning` package [@???].
...
# Add topic pseudo-names based on the top 5 words in each topic
...
topicToFilter <- grep('network', topicNames)[1] 
topicThreshold <- 0.2
...
vizDataFrame <- melt(cbind(data.frame(topic_prop_per_year), 
                           document = factor(rownames(topic_prop_per_year))), 
                     variable.name = "topic", id.vars = "document")  

# plot topic proportions per decade as bar plot
vizDataFrameWithYear <- vizDataFrame |> 
  left_join(select(emu_theses, title, graduation_year), by = c("document" = "title")) |> 
  ggplot(aes(x=factor(graduation_year), y=value, fill = topic)) + 
  geom_bar(stat = "identity", position = "fill") +
  scale_fill_viridis_d(name = "Topics") +
  xlab("Graduation year") +
  ylab("Proportion of topics") +
  labs(title = "Proportion of summed topic probabilities over the years of the EMU program")
...
LDAvis::serVis(emu_lda_json, out.dir = "vis", open.browser = FALSE)
...
get_top_words_per_corpus(data = emu_theses_words |> 
                           mutate(word = textstem::lemmatize_words(emu_theses_words$word)), top_n = n) |> 
