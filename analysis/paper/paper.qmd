...
urbanism_sw <- urbanism_stopwords(
    add_stopwords = c("emu", "european postgraduate master in urbanism", 
                      "european post master in urbanism",
                      "european postmaster in urbanism",
                      "tu delft", "ku leuven", "upc barcelona", "iuav venice"))
thesis_sw <- thesis_stopwords(add_stopwords = c("advisor", "prof", "fig", "ure", "ning"))
custom_sw <- c("space", "spatial", "spaces", "plan", "public", "development") |>
  (\(.) paste0("\\b", ., "\\b"))() |> 
  paste(collapse = "|")
...
emu$text_clean <- emu |> 
  c_ngrams(n_max = 3, text_col = "text_clean",
           title_col = "title", min_freq = 20, lemma = TRUE) 
...
# TODO Replace title column with ID column
