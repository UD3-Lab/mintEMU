...
emu_theses <- read_csv(here::here("analysis","data", "raw_data", "theses-metadata.csv")) %>%
  mutate(text = "")

pdf_names <- dir(here::here("analysis", "data", "raw_data"), pattern = "*.pdf")

emu_theses <- emu_theses %>%
  filter(file_name %in% pdf_names)
  
pdf_paths <- here::here("analysis","data", "raw_data", emu_theses$file_name)
...
Stop words:
- Custom stop words from titles and author names should only be removed from corresponding theses;
...
- All theses were written in English; for common English stop words the "snowball" lexicon was used.
...
emu_theses_top <- emu_theses_words %>%
  group_by(title) %>%
...
emu_theses_top[emu_theses_top$word == "ring",]
