...
# TODO replace the following lines with the metadata included in the package
# emu_metadata <- data(emu_metadata)
## Read metadata for all theses
all_theses <- read_xlsx(path = here(data_path, "theses-all.xlsx"))

## Read only metadata for theses with PDF available
emu <- read_csv(here(data_path, "theses-metadata.csv")) |>
  mutate(text_raw = "") |>
  filter(!is.na(pdf_via))

# TODO replace this and next code chunk with reading `text_raw` from the package
# emu <- data(emu_raw)
pdf_paths <- here(data_path, emu$file_name)
```
...
if (!file.exists(data_path)) {
  ...
  emu$text_raw <- convert_pdf_text(pdf_paths)
  
  # Add latitude and longitude information to theses data frame
  emu <- geocode_thesis_locations(emu)

  # Write thesis dataframe to file
  write_csv(emu, data_path)
} else {
  emu$text_raw <- read_csv(data_path)$text
  emu$longitude <- read_csv(data_path)$longitude
  emu$latitude <- read_csv(data_path)$latitude
}
...
```{r clean-text}
#| echo: false
#| dependson: get-pdf-text

emu$text_clean <- emu$text |>
  clean_basic() 
...
A total number of `r nrow(emu)` out of `r nrow(all_theses)` were analysed.
...
get_top_words_per_document(data = emu_words, top_n = 3)
```
...
## Bigrams
emu_bigrams <- emu |> 
  get_ngrams(n = 2, title_col = "title", text_col = "text_clean", lemma = TRUE) |> 
  unite(ngram_lemma, w_1_lemma, w_2_lemma, sep = "_", remove = FALSE)
  
bigram_counts <- emu_bigrams |> 
  count(ngram, sort = TRUE) |> 
  filter(n > 20)

emu_bigrams <- emu_bigrams |> 
  filter(ngram %in% bigram_counts$ngram)

bigram_lemma_counts <- emu_bigrams |> 
  count(w_1_lemma, w_2_lemma, sort = TRUE) |> 
  filter(n > 5)

# Concatenate bigrams

# # First attempt using a named vector in `str_replace_all()`
c_bigrams <- paste(emu_bigrams$ngram_lemma, sep = "_")
names(c_bigrams) <- emu_bigrams$ngram

system.time(
  emu$text_clean <- emu$text_clean |>
    str_replace_all(fixed(c_bigrams))
)

# Second attempt with `mgsub::mgsub()`
# emu$text_clean <- mgsub(emu$text_clean,
#                         pattern = emu_bigrams$ngram,
#                         replacement = emu_bigrams$ngram_lemma)
...
## Trigrams
emu_trigrams <- emu |> 
  get_ngrams(n = 3, title_col = "title", text_col = "text_clean", lemma = TRUE)

trigram_counts <- emu_trigrams |> 
  count(w_1, w_2, w_3, sort = TRUE)

trigram_lemma_counts <- emu_trigrams |> 
  count(w_1_lemma, w_2_lemma, w_3_lemma, sort = TRUE)
