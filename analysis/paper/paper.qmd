...
-   Evaluating the number of topics quantitatively requires can be done through
the measure of **topic coherence**
...
topicProportions <- colSums(theta_5) / nDocs(emu_dtm)  # mean probabilities over all paragraphs
names(topicProportions) <- topic_names_5     # assign the topic names we created before
...
names(countsOfPrimaryTopics) <- topic_names_5
...
topicToFilter <- grep('river', topic_names_5)[1] 
topicThreshold <- 0.2
selectedDocumentIndexes <- which(theta_5[, topicToFilter] >= topicThreshold)
filteredCorpus <- emu[selectedDocumentIndexes,]
...
vizDataFrameWithYear <- melt(cbind(data.frame(topic_prop_per_year), 
                     ...
                     variable.name = "topic", id.vars = "document") |> 
    left_join(select(emu, ID, "year" = graduation_year), by = c("document" = "ID"))
