...
emu_theses$text_clean <- emu_theses$text |>
  ...
  str_remove_all(subject_stopwords()) |> 
  str_remove_all(document_stopwords())
```
...
urbanism_stopwords <- urbanism_stopwords() 
thesis_stopwords <- thesis_stopwords()

emu_theses_words <- emu_theses |> 
  select(title, text_clean) |> 
  unnest_tokens(output = word, input = text_clean) |>   # remove punctuation, convert to lowercase, seperate all words
  anti_join(stop_words, by = "word") |> 
  anti_join(urbanism_stopwords, by = "word") |> 
  anti_join(thesis_stopwords, by = "word")
```
