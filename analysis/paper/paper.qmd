...
abstract: |
  After seventeen years of being part of the the European post-Master in 
  Urbanism (EMU), Delft Univerisity of Technology (TU Delft) left the program in
  2021. This paper aims to describe the legacy of TU Delft's contribution to the
  EMU and the profession. It reports on a topic model of graduation theses
  produced during the program. Results reveal topics that are consistent with
  the set-up of the program in two thematic semesters and three possible
  exchange semester locations. The paper discusses the relevance of the findings
  for the state of the profession and education in the field.
# TODO check sentences on results and discussion
# TODO JPER abstracts should generally be no longer than 125 words and have one sentence to
...
The research presented in this paper was prompted by the closure of the European post-Master in Urbanism (EMU) at TU Delft in 2021. The EMU is an advanced program started jointly by TU Delft, KU Leuven, UPC Barcelona and Università IUAV di Venezia in 2005. The program distinguished itself internationally as an advanced post-master open only to applicants with a prior MSc degree and practical experience in urbanism or a related field. With a combined research and design curriculum, it provided a bridge towards both advanced practice and research trajectories after graduation. All students followed an exchange semester at one of the other three program universities, allowing them to have a broad, international view on the field of urbanism. Although located in Europe and named "European", its geographic scope was global (see @fig-map-theses), visible especially at the stage of graduation, considering that a large majority of students were of non-European origin and chose to work on a case from their home location.
...
## A text mining approach {#tmapproach}
...
<!-- Table shown in html output -->

::: {.content-visible when-format="html"}
```{r}
#| label: tbl-data-selection-html
#| tbl-cap: Overview of theses included in the analysis
#| tbl-cap-location: top

theses_overview |>
  mutate(`% of theses per period` = formattable::color_tile("lightpink", "lightgreen")(readr::parse_number(theses_overview$`% of theses per period`))) |>
  rename("Count" = `No. theses included`, "Percentage" = `% of theses per period`) |> 
  kableExtra::kable(format = "html", align = "lrrr", escape = FALSE) |>
  kableExtra::column_spec(4, width = "3cm") |> 
  kableExtra::row_spec(nrow(theses_overview), bold = TRUE) |>
  kableExtra::kable_styling("hover", full_width = FALSE) |> 
  kableExtra::add_header_above(c(" " = 2, "Included" = 2))
```
:::

<!-- Table shown in non-html output -->
...
theses_overview |> 
  gt::gt() |> 
  gt::tab_options(table.width = "300")

```
:::

```{=html}
<!-- TODO Replace with interactive map when rendering HTML.
This should be an argument in the `visualise_thesis_locations()` function. -->
```
::: {.content-visible when-format="html"}
```{r}
...
3.  From the metadata only the title, graduation year, and location of the case studied in the theses were kept. Column names were shortened and updated to follow the naming conventions used in the dataset.
...
Text mining is a process of extracting implicit information from unstructured textual data [@jo2019]. following the steps of tokenisation, construction of a Document-Term Matrix (DTM), fitting a topic model and visualisation of the results.
...
```{r}
k.topics <- 2:9
folding <- c(rep(1:5, each = 13), rep(5, 3))

runonce <- function(k, fold) {
    testing.dtm <- which(folding == fold)
    training.dtm <- which(folding != fold)
    
    training.model <- topicmodels::LDA(dtm[training.dtm, ], k = k)
    test.model <- topicmodels::LDA(dtm[testing.dtm, ], model = training.model, control = list(estimate.beta = FALSE))
    
    topicmodels::perplexity(test.model)
}
...
lda <- topicmodels::LDA(dtm, k = 5, method="Gibbs",
...
topic_pseudonames <- name_topics(beta, n_words = 5)
...
colnames(topic_prop) <- paste0(paste0("Topic ", 1:length(topic_pseudonames), ": "), topic_pseudonames)
...
#### Topic 3: `r topic_pseudonames[3]` {#topic3}
...
#### Topic 4: `r topic_pseudonames[4]` {#topic4}

**Integrated projects:** *park build main house community*

#### Topic 5: `r topic_pseudonames[5]` {#topic5}
...
### The dynamics of thesis topics {#topdyn}
...
colnames(topic_prop_per_year) <- topic_pseudonames
