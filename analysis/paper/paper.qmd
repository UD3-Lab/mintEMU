...
# Create sliding window function to calculate skipgram probabilities,
# that is, how often each word occurs near each other word.
# A small window size of 3-4 shows how a word is used, while a larger  
# window size of, say, 10 gives more information about the domain topic.
slide_windows <- function(tbl, window_size) {
  ...
  out |> 
    transpose() |>  # Transpose `result` and `error` into columns,
    pluck("result") |>  # only keep `result`,
    compact() |>  # remove any NULL values [is this for the last windows?],
    bind_rows()  # and bind all rows into a single data frame.
}
```
...
# To calculate how often words occur on their own or tigether with other words
...
# The resulting matrix will be of size n_voc * n_dim instead of n_voc * n_voc
